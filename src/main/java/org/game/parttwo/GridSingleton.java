...
@Getter
@Setter
public class GridSingleton extends ArrayList<ArrayList<Cell>> {
    private static volatile GridSingleton map;
    ...
    private GridSingleton(int length, int width) {
        gridLength = length;
        gridWidth = width;
    ...
    public static GridSingleton generateMap(int length, int width) {
        Random rand = new Random();

        GridSingleton result = map;
        if (result == null) {
            synchronized (GridSingleton.class) {
                result = map;
                if (result == null) {
                    map = result = new GridSingleton(length, width);
                }
            }
        }

        for (int i = 0; i < 2; i++) {
            setCellType(result, CallEntityType.SANCTUARY, rand);
        }

        for (int i = 0; i < 4; i++) {
            setCellType(result, CallEntityType.ENEMY, rand);
        }

        setCellType(result, CallEntityType.PORTAL, rand);

        int remainingCells = gridLength * gridWidth - 7;
        int randomCell = rand.nextInt(remainingCells);
        for (int i = 0; i < randomCell; i++) {
            setCellType(result, getRandomCallEntityType(), rand);
        }
    ...
    public static void setCellType(GridSingleton grid, CallEntityType type, Random rand) {
        while(true) {
    ...
    public void setCellType(GridSingleton grid, CallEntityType type, int row, int col) {
        Cell cell = grid.get(row).get(col);
        cell.setType(type);
